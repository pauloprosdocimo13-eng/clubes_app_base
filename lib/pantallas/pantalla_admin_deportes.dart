...
class _PantallaAdminDeportesState extends State<PantallaAdminDeportes> {
  ...
  void dispose() {
    ...
    _categoriasManualController.dispose();
    super.dispose();
  }
  ...
  Future<void> _cargarDeportes() async {
    try {
      ...
      setState(() {
        if (doc.exists &&
            doc.data() != null &&
            doc.data()!.containsKey('menu_deportes')) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error cargando deportes: $e')));
      }
    }
  }

  void _mostrarDialogo({
    Map<String, dynamic>? deporteEditar,
    int? indexEditar,
  }) {
    final bool esEdicion = deporteEditar != null;
    ...
    if (esEdicion) {
      ...
      final List cats = deporteEditar['categorias'] ?? [];
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setStateDialog) {
          return AlertDialog(
            title: Text(
              esEdicion ? 'Editar Tira/Actividad' : 'Nueva Tira/Actividad',
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_mostrarManual) ...[
                    const SizedBox(height: 15),
                    const Text(
                      'Escribe las categorías separadas por coma:',
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(
                        border: OutlineInputBorder(),
                        hintText: 'Ej: Primera, Reserva, Sub-20',
                      ),
                    ),
                  ] else ...[
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 5),
                          Text(
                            (_plantillas[_plantillaSeleccionada]!['categorias']
                                    as List)
                                .join(', '),
  ...
  Future<void> _guardarDeporte(int? indexEditar) async {
    ...
    final List<dynamic> nuevaLista = List.from(_deportes);
    ...
    try {
      await ServicioDatosClub.configuracionDoc(
        'general',
      ).set({'menu_deportes': nuevaLista}, SetOptions(merge: true));
  ...
  Future<void> _borrarDeporte(int index) async {
    ...
    try {
      final queryJugadores = await ServicioDatosClub.jugadores
          .where('deporte_id', isEqualTo: idDeporte)
          .count()
          .get();

      final queryPartidos = await ServicioDatosClub.partidos
          .where('deporte_id', isEqualTo: idDeporte)
          .count()
          .get();
        ...
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text(
              ...
              'Por seguridad, debes eliminar primero los jugadores y '
              'partidos asociados, o moverlos a otra tira antes de '
      ...
      final confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(
            "Se eliminará '$titulo' del menú. Como no tiene datos "
            'asociados, es seguro continuar.',
          ),
          actions: [
            TextButton(
      ...
      if (confirm == true) {
        ...
        final List<dynamic> nuevaLista = List.from(_deportes)..removeAt(index);
      ...
      debugPrint('Error al verificar deporte: $e');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _cargando
          ...
          ? const Center(
              child: Text(
                'No hay tiras o actividades configuradas.',
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(
                      backgroundColor: tipo == 'competencia'
                          ? Colors.green[100]
                          : Colors.purple[100],
                      child: Icon(
                        tipo == 'competencia'
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () => _mostrarDialogo(
                            deporteEditar: Map<String, dynamic>.from(d as Map),
                            indexEditar: index,
                          ),
                        ),
                        IconButton(
