...
class PantallaPruebaReservasAdmin4F extends StatefulWidget {
  final ConfiguracionApp config;

  const PantallaPruebaReservasAdmin4F({
    super.key,
    required this.config,
  });

  @override
  State<PantallaPruebaReservasAdmin4F> createState() =>
      _PantallaPruebaReservasAdmin4FState();
}

class _PantallaPruebaReservasAdmin4FState
    extends State<PantallaPruebaReservasAdmin4F> {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (rol != 'superadmin' &&
        ...
        throw Exception('El usuario no tiene acceso al club.');
      }
      ...
      setState(() {
        ...
        _estado =
            ...
            'Espacios y reservas listos para probar.';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _verificar() async {
    ...
    try {
      final espacios = await ServicioDatosClub.espacios.limit(10).get();
      final reservas = await ServicioDatosClub.reservas.limit(10).get();
      final vencimientos =
          await ServicioDatosClub.vencimientos.limit(10).get();
      final config =
          await ServicioDatosClub.reservasConfiguracion.get();
      ...
      setState(() {
        _resultado =
            ...
            'Espacios: ${espacios.docs.length}\n'
            'Reservas: ${reservas.docs.length}\n'
            'Vencimientos: ${vencimientos.docs.length}\n'
            'WhatsApp: ${config.data()?['telefono_wsp'] ?? ''}';

        _estado = '✅ Datos centrales leídos correctamente.';
      });
    } catch (e) {
      if (!mounted) return;
      setState(() => _estado = '❌ $e');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('TuSede · Prueba 4F-2H Admin'),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (!_sesionActiva)
                      ElevatedButton(
                        onPressed: _procesando ? null : _login,
                        child: const Text(
                    ...
                    if (_sesionActiva) ...[
                      ElevatedButton.icon(
                        onPressed: () => Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => PantallaAdminEspacios(
                      ...
                      ElevatedButton.icon(
                        onPressed: () => Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => PantallaAdminCalendario(
                              config: widget.config,
                            ),
                          ),
                        ),
