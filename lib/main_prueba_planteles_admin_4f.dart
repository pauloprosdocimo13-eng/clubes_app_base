...
import 'pantallas/pantalla_admin_jugadores.dart';
...
Future<void> main() async {
  ...
  runApp(_PruebaPlantelesAdminApp(config: config, centralOk: centralOk));
}

class _PruebaPlantelesAdminApp extends StatelessWidget {
  final ConfiguracionApp config;
  final bool centralOk;

  const _PruebaPlantelesAdminApp({
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      title: '4F-2L-B Planteles Admin',
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: config.colorPrimario),
      ),
      home: _PruebaPlantelesAdmin(config: config, centralOk: centralOk),
    );
  }
}

class _PruebaPlantelesAdmin extends StatefulWidget {
  final ConfiguracionApp config;
  final bool centralOk;

  const _PruebaPlantelesAdmin({required this.config, required this.centralOk});

  @override
  State<_PruebaPlantelesAdmin> createState() => _PruebaPlantelesAdminState();
}

class _PruebaPlantelesAdminState extends State<_PruebaPlantelesAdmin> {
  ...
  Future<void> _verificar() async {
    ...
    try {
      ...
      final jugadores = await ServicioDatosClub.jugadores.get();
      ...
      if (mounted) {
        setState(() {
          _resultado =
              ...
              'Jugadores/DT encontrados: ${jugadores.size}\n'
              'Colección: ${ServicioDatosClub.jugadores.path}\n'
              'Ruta esperada: '
      ...
      if (mounted) {
        setState(() {
          _resultado = 'Error verificando planteles: $e';
        });
      }
    } finally {
      if (mounted) {
  ...
  void _gestionarPlanteles() {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => PantallaAdminJugadores(config: widget.config),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Prueba 4F-2L-B - Planteles Admin'),
        ...
        child: ConstrainedBox(
          ...
          child: ListView(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              ElevatedButton.icon(
                onPressed: _cargando ? null : _gestionarPlanteles,
                icon: const Icon(Icons.groups),
                label: const Text('3. GESTIONAR PLANTELES / JUGADORES'),
              ),
              const SizedBox(height: 12),
              ElevatedButton.icon(
