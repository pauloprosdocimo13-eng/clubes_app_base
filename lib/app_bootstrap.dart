...
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  final sabor = const String.fromEnvironment('FLAVOR', defaultValue: 'guemes');
  await Firebase.initializeApp(options: RegistroFlavors.firebaseOptionsDe(sabor));
...
Future<void> bootstrapApp(String sabor) async {
  ...
  runApp(MiAplicacion(config: config, navigatorKey: navigatorKey));
}
...
void _manejarRedireccion(RemoteMessage message, ConfiguracionApp config) {
  ...
  if (tipo == 'noticia') {
    nav.push(MaterialPageRoute(builder: (_) => PantallaNoticias(config: config)));
  } else if (tipo == 'aviso') {
    nav.push(
      MaterialPageRoute(
        builder: (_) => PantallaAvisos(config: config, deporteId: 'general'),
      ),
    );
  }
}
